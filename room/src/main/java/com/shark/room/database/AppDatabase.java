package com.shark.room.database;
...
import com.shark.room.database.dao.EmDao;
import com.shark.room.database.dao.UserDao;
import com.shark.room.database.entity.User;
import com.shark.room.database.entity.UserEm;
import com.shark.room.database.help.DateConverter;
 ...
 * desc   :
 */
// 包含数据库持有者，并充当与应用程序持久化的、关系型的数据的底层连接的主要访问点。
...
public abstract class AppDatabase extends RoomDatabase {
     ...
     * 获取数据库持有者
     * @param context 上下文
     * @return 包含数据库持有者，并充当与应用程序持久化的、关系型的数据的底层连接的主要访问点。
     */
    public static AppDatabase getDatabase(final Context context) {
        if (INSTANCE == null) {
            synchronized (AppDatabase.class) {
                if (INSTANCE == null) {
                    // 内存临时数据库
                    // INSTANCE = Room.inMemoryDatabaseBuilder(context.getApplicationContext(), UserRoomDatabase.class).build();
                    // 持久性数据库
