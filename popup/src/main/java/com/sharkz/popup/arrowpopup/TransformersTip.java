package com.sharkz.popup.arrowpopup;
...
public abstract class TransformersTip extends PopupWindow implements PopupWindow.OnDismissListener {
    ...
    private View mAnchorView; // 锚点控件 触发popup的View
    private int mTipGravity; // 浮窗相对于锚点控件展示的位置 （整个弹窗在锚点控件的位置）
    ...
    /**
     * 自动显示  内部处理了 显示的位置  调用当前 前面的某些 set方法将 无效
     * .setTipGravity(TipGravity.TO_BOTTOM_CENTER) // 设置浮窗相对于锚点控件展示的位置
     * .setArrowGravity(ArrowGravity.CENTER_TO_START) // 设置箭头相对于浮窗的位置
     */
    public TransformersTip autoShow() {
        RectF re = calculateRectInWindow(mAnchorView);
        // 获取屏幕的 高度
        DisplayMetrics outMetrics = new DisplayMetrics();
        ((Activity) mAnchorView.getContext()).getWindowManager().getDefaultDisplay().getMetrics(outMetrics);
        int widthPixels = outMetrics.widthPixels;
        int heightPixels = outMetrics.heightPixels;
        if ((re.bottom) > (heightPixels / 2)) {
